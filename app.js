...
const JOG_SPEED = 6000;         // cuánto alejarse del centro en un jog
const JOG_MS = 250;             // cuánto tiempo dura el pulso de jog
...
class MKBleAdapter {
  ...
  async probeField(field, ms = 500) {
    ...
    if (field === "C") {
      ...
      machineVel[field] = VEL_STOP + JOG_SPEED; // alejarse del centro
    }
  ...
  /** Jog: pulso de velocidad en un sentido durante JOG_MS, después frena.
   * direction: +1 / -1. invert cambia el sentido físico. */
  async sendMove(axis, direction, ms = JOG_MS) {
    ...
    await this._emitFrame();
    await new Promise((r) => setTimeout(r, ms));
    machineVel[field] = VEL_STOP; // frenar este eje
    await this._emitFrame();
    // estimación de avance a lazo abierto (para mostrar/fijar límites)
    estPos[axis] += sign * 1; // 1 "paso de jog" por pulso
  }
